...
function surgeGate(d) {
  ...
  if (cur.as_of && prev.as_of && cur.as_of === prev.as_of) {
    // **この日はメールを送らない。**
    // `_prev` は送信後に更新するので、as_of が一致するのは
    // 「前回送ったのと同じ報のまま、新しい公式報が出ていない」ことを意味する。
    // 以前は「本文にその旨を明記すること」と書いていたが、これだと
    // 前回と同じ数値のメールを毎日重ねて出すよう読める。研究部には雑音になる。
    // 公式の新しい報が無い日は、送らないのが正しい。
    reasons.push(
      `as_of が前報と同一（${cur.as_of}）— 一次情報に新しい報が出ていない。` +
      `**この日はメールを送らない。** 前回送ったのと同じ数値を重ねて出さないため。` +
      `これは失敗ではない。「新しい報が無いため送信を見送った」と報告して終わること`);
  }
