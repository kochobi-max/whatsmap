...
const TO = [
  ...
  '  const PRM_AVAIL = (BI ? 6.72 : 6.60) - 1.35;   // 表に使える高さ',
  ...
  '    } else {',
  '      // 1言語版は他の表と同じ estRowH で測る。行送りは実測値（apply_table_fit.js）。',
  '      // 独自の見積もりを持つと、直すたびに片方だけ直り続ける。',
  '      let h = 0.38;                                                // 見出し行',
  '      for (const r of prm) h += estRowH([[r[0], PRM_LW, prmFs, 4], [r[1], PRM_VW, prmFs, 4]], 0.30);',
  '      if (h <= PRM_AVAIL) break;',
  '    }',
  '  }',
  '  // rowH は「最低の高さ」。1言語版では 0.30 に下げ、行ごとの実際の高さに任せる。',
  '  // 高めに固定すると、折り返さない行まで場所を取り、表全体が下へあふれる。',
  '  const prmRowH = BI ? Math.max(0.30, Math.min(0.54, PRM_AVAIL / (prm.length + 1))) : 0.30;',
  '  s.addTable([[tableHeaderCell("Item / 項目"), tableHeaderCell("Value / 値")]].concat(',
  '    prm.map((r, i) => [',
