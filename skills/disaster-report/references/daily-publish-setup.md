...
## 5-1. 定期タスクの既定は、ノートPCに合っていない（2026-09-07）

`schtasks /create` で作ったタスクの既定は、次のようになっている。

| 項目 | 既定 | 何が起きるか |
|---|---|---|
| バッテリで開始しない | **有効** | 電源に繋いでいない朝は**そもそも走らない** |
| バッテリ モードで停止 | **有効** | 実行中に電源が抜けると止まる |
| ログオン モード | 対話型のみ | 荒木田さんがログオンしていないと走らない |
| 取りこぼしたら後で実行 | 有効 | 遅れて走る（**これは望ましい**） |

2026-09-07、`schtasks /query /tn "ADRC disaster report daily" /v /fo LIST` で確認した。
同日、8:10 の回が **9:48 に遅れて走った**（前回の結果 0）。これは取りこぼしの追いつきで、
正常な動きである。**「なぜ今ごろ走ったのか」と疑う必要はない。**

**「PCが動いていない」と報告した日の一部は、電源が切れていたのではなく
バッテリ駆動だった可能性がある。** ここを見ていなかった。

### バッテリ駆動でも走るようにした（荒木田さんの了解・2026-09-07）

`publish_local.js --setup`（＝`ADRC_setup_and_publish.bat`）が、登録済み・新規の
どちらの場合も次を適用する。**打ち込んでもらう手順にはしない。**
...
適用後に読み直して `battery-start=True stop-on-battery=False catch-up=True` を確認し、
表示する。**失敗しても止めない。** 設定を変えられなくても公開そのものは済んでいる。
その場合は、タスクスケジューラの「条件」タブで
「コンピューターをAC電源で使用している場合のみ...」のチェックを外す旨を案内する。

**「対話型のみ」はそのままにしてある。** ログオンなしで走らせるにはパスワードの保存が
必要になる。朝いちばんにPCを開く運用と噛み合っており、取りこぼしても追いつく。
