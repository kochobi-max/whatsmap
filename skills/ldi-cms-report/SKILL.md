...
description: >
  ...
  CMS担当者へメールを自動送信する（更新0件の日も「本日更新なし」を送る）。
...
> 数値は暫定。熊本（M7.1・国内）、コロンビア・チョコ（M7.4）、インドネシアNTT（M7.7・EO＋チャーター）
> がいずれも該当することを確認済み。運用しながら調整する。
...
- **宛先**: CMS登録担当者（既定: `td-date@adrc.asia`）／ Cc: `ma-arakida@adrc.asia`
...
| **更新0件** | `ADRC LDI CMS入力データ（YYYY-MM-DD）本日の更新対象なし（全N件確認済み）` |
...
**必ず送る。沈黙にしてはならない。** 担当者はスクリプトが止まったのか本当に更新がないのかを判別できない。
