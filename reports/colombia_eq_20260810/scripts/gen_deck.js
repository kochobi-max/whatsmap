...
const CELL = {
  ...
  "Presidency / Gobernación del Chocó": { ja: "大統領府／チョコ県", es: "Presidencia / Gobernación del Chocó" },
  "Presidency of Colombia, Decree 1172 of 12 Aug": { ja: "コロンビア大統領府 大統領令1172号（8月12日）", es: "Presidencia de Colombia, Decreto 1172 del 12 de agosto" },
  "Gobernación del Chocó, 12 Aug": { ja: "チョコ県（8月12日）", es: "Gobernación del Chocó, 12 de agosto" },
  ...
  "UNGRD, 18 Aug": { ja: "UNGRD（8月18日）", es: "UNGRD, 18 de agosto" },
  "UNGRD, 16 Aug 18:30": { ja: "UNGRD（8月16日18:30）", es: "UNGRD, 16 de agosto 18:30" },
  ...
  "Alcaldía Mayor de Bogotá, 16 Aug": { ja: "ボゴタ特別市（8月16日）", es: "Alcaldía Mayor de Bogotá, 16 de agosto" },
...
heading(s, "Human Damage", "人的被害");
{
  const big = [
    [T("Deaths", "死者"), "312", RED],
    [T("Injured", "負傷者"), T("4,611", "4,611"), "9A3B12"],
    [T("Missing", "行方不明"), "290", NAVY],
  ];
  big.forEach((b, i) => {
    const x = 0.4 + i * 4.2;
...
{
  const big = [
    [T("Reported to the authorities", "当局への届出"), "290", RED],
...
{
  const items = d.support_international;
  const nP = Math.max(1, Math.ceil(items.length / 8));
  const per = Math.ceil(items.length / nP);
  for (let i = 0; i < items.length; i += per) {
    const chunk = items.slice(i, i + per);
    const last = i + per >= items.length;
    const suffix = nP > 1 ? ` (${Math.floor(i / per) + 1}/${nP})` : "";
    s = newSlide();
    heading(s, "International Support", "国際的な支援", suffix);
    bulletsTier(s, 0.4, 1.12, 12.5, last ? 4.2 : 5.7, chunk, { base: 11.5 });
    if (last) noteBox(s, 0.4, 5.42, 12.5, 1.38, L(d, "support_intl_note_title"), L(d, "support_intl_note"));
